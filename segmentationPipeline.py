        ...
        for i, mask in enumerate(masks):
            if hasattr(results.boxes, "cls"):
                ...
                if int(class_id) in skuMapping:
                    ...
                    raise KeyError(
                        f"Class ID {class_id} not found in the specified SKU mapping."
                    )
            else:
            ...
            mask_resized = cv2.resize(
                mask_binary, (orig_img.shape[1], orig_img.shape[0])
            )
    ...
    def rowWiseSegmentation(self):
        ...
        for imageName in os.listdir(self.inputImageDir):
            ...
            if imageName.lower().endswith((".jpg", ".jpeg", ".png")):
                ...
                orig_img, processed_masks, _, _ = self.process_masks(
                    self.rowModel, imagePath, self.rowSku
                )
                for mask, class_name in processed_masks:
                    white_background = np.ones_like(orig_img) * 255
                    for c in range(3):
                        white_background[:, :, c] = np.where(
                            mask == 1, orig_img[:, :, c], 255
                        )
                    output_path = os.path.join(self.rowImageDir, f"{class_name}.jpg")
                    cv2.imwrite(
                        output_path, white_background, [cv2.IMWRITE_JPEG_QUALITY, 95]
                    )
                    logging.info(
                        f"Saved segmented image for SKU '{class_name}' at: {output_path}"
                    )
                ...
                for mask, class_name, confidence in unique_masks:
                    ...
                    for contour in contours:
                        ...
                        output_path = os.path.join(
                            self.sectionImageDir, f"{class_name}%.jpg"
                        )
                        cv2.imwrite(
                            output_path,
                            black_background,
                        ...
                        logging.info(
                            f"Saved unique cropped image for SKU '{class_name}' at: {output_path}"
                        )
    ...
    def productSegmentation(self, section_dict):
        ...
        for imageName in os.listdir(self.sectionImageDir):
            ...
            if imageName.lower().endswith((".jpg", ".jpeg", ".png")):
                ...
                for i, (mask, class_name) in enumerate(processed_masks):
                    ...
                    for j, (u_mask, _, u_conf) in enumerate(unique_masks):
                        ...
                        if mask.shape != u_mask.shape:
                            logging.warning(
                                f"Mask shapes do not match: {mask.shape} vs {u_mask.shape}"
                            )
                            continue
                ...
                for mask, class_name, confidence in unique_masks:
                    white_background = (
                        np.ones_like(orig_img) * 255
                    )  # Create a white background
                    for c in range(3):  # Apply mask to each color channel
                        white_background[:, :, c] = np.where(
                            mask > 0, orig_img[:, :, c], white_background[:, :, c]
                        )

                    # Save the image
                    base_name = os.path.splitext(imageName)[0]
                    output_path = os.path.join(
                        self.productImageDir, f"{base_name}_{class_name}.jpg"
                    )
                    cv2.imwrite(
                        output_path, white_background, [cv2.IMWRITE_JPEG_QUALITY, 95]
                    )
